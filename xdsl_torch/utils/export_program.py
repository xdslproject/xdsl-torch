...
def export_program(func_op: func.FuncOp) -> torch.fx.Graph:
    ...
    for op in func_op.body.ops:
        ...
        if type(op) is func.ReturnOp:
            graph.create_node("output", "", tuple(nodes[arg.name_hint if arg.name_hint else ""] for arg in op.operands))
        else:
            nodes[op.results[0].name_hint if op.results[0].name_hint else ""] = graph.create_node(
                "call_function",
                REVERSE_XDSL_TORCH_OPS[type(op)],
                tuple(nodes[arg.name_hint if arg.name_hint else ""] for arg in op.operands),
                None,
